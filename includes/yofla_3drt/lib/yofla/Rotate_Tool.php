...
class Rotate_Tool
{
    ...
    const IMAGES_DIRECTORY_SMALL = 'imagessmall';
    ...
    private function _check_cache_directories()
    {
       ...
       $cache_dir_paths[] = self::$system_path.'cache/configs';
       $cache_dir_paths[] = self::$system_path.'cache/pages';
    ...
    public function get_iframe($path,$options=null)
    {
        ...
        $width = self::$settings["player"]["width"];
        $height = self::$settings["player"]["height"];
        $iframe_style = self::$settings["player"]["iframeStyle"];
        ...
        return self::_get_template_output('iframe_tag.tpl',$values);
    ...
    public function get_iframe_url($path)
    {
        ...
        $dynamic_url = self::$system_url."lib/yofla/get_page_for_iframe.php?p=".urlencode($path);
        ...
        else
        {
            ...
            if (file_exists($path_to_cached_page))
            {
    ...
    public function get_page_for_iframe($path)
    {
        ...
        $config_url = self::get_config_url_for_product($path);
        ...
        //store content to cache, if config_url is cached
        if(strpos($config_url,"cache/configs") >= 0)
        {
    ...
    /**
     * Returns a link to a script that generates config js file based on provided path.
     * Checks if cached version exists first.
     *
    ...
    public function get_config_url_for_product($path)
    {
        ...
        //dynamic url for php script that is returning config.js content
        $data = array("p"=>$path);
